...
/**
 * Komponen Index untuk Stock Opname
 * Menampilkan daftar stock opname records dengan filter, statistik, dan action buttons
 *
 * @param {Object} props - Props dari backend
 * @param {Array} props.stockOpnameRecords - Array stock opname records
 * @param {Object} props.statistics - Statistik stock opname
 * @param {Object} props.filters - Filter aktif
 * @param {Object} props.pagination - Metadata pagination
 */
const Index = ({ stockOpnameRecords, pagination, statistics, filters }) => {
  const { props } = usePage();
  ...
  const handleCreate = useCallback(() => {
    router.get(route('stock-opname.create'));
  }, []);

  /**
   * Handle edit stock opname record
   * @param {string} id - ID stock opname record
   */
  const handleEdit = useCallback((id) => {
    router.get(route('stock-opname.edit', id));
  }, []);

  /**
   * Handle submit stock opname record
   * @param {Object} record - Stock opname record object
   */
  const handleSubmit = useCallback((record) => {
    if (confirm('Apakah Anda yakin ingin mengirim stock opname ini? Tindakan ini tidak dapat dibatalkan.')) {
      const items = (record.items || []).map((item) => ({
        product_variant_id: item.product_variant_id,
        physical_stock: item.physical_stock,
      }));

      // Format updated_at ke Y-m-d H:i:s untuk validasi backend
      const updatedAt = typeof record.updated_at === 'string'
        ? record.updated_at.replace('T', ' ').substring(0, 19)
        : new Date(record.updated_at).toISOString().replace('T', ' ').substring(0, 19);

      router.post(route('stock-opname.submit', record.id), {
        items,
        ...
        onFinish: () => setLoading(false),
        onSuccess: () => {
          router.reload({ only: ['stockOpnameRecords', 'statistics'] });
        },
        onError: (errors) => {
          const firstError = Object.values(errors)[0];
          alert(Array.isArray(firstError) ? firstError[0] : 'Gagal submit stock opname');
        },
      });
    }
  }, []);
  ...
  /**
   * Handle delete stock opname record
   * @param {Object} record - Stock opname record yang akan dihapus
   */
  ...
  /**
   * Konfirmasi delete stock opname record
   */
  const confirmDelete = useCallback(() => {
    if (deleteConfirm) {
      router.delete(route('stock-opname.destroy', deleteConfirm.id), {
        onSuccess: () => {
          router.reload({ only: ['stockOpnameRecords', 'statistics'] });
  ...
  /**
   * Batal delete stock opname record
   */
   ...
   * @param {Object} record - Stock opname record
   */
  ...
  const handleSaveNote = useCallback(async () => {
    ...
    try {
      const response = await axios.put(
        route('stock-opname.update-note', noteModal.recordId),
      ...
      if (response.data.success) {
        ...
        router.reload({ only: ['stockOpnameRecords'] });
   ...
   * @param {string} recordId - Stock opname record ID
   */
  const handleRemoveNote = useCallback(async (recordId) => {
    ...
    try {
      const response = await axios.put(
        route('stock-opname.update-note', recordId),
        { note: null }
      );
  ...
  /**
   * Render status badge
   * @param {string} status - Status stock opname
  ...
  /**
   * Render stock opname card untuk mobile dan desktop
   * @param {Object} record - Stock opname record
   * @returns {JSX.Element} Stock opname card component
   */
  const renderStockOpnameCard = useCallback((record) => {
            ...
            <div className="flex-1">
              ...
              {record.creator && (
                <div className="mt-1 text-sm text-gray-500">
                  Dibuat oleh: {record.creator.name || record.creator}
                </div>
              )}
            </div>
          </div>
        ...
        <MobileCardFooter>
          <div className="flex flex-col gap-3 w-full">
            ...
            <div className="flex gap-2 w-full">
              <Link
                href={route('stock-opname.show', record.id)}
              ...
              {isDraft && can('stock_opname.submit') && (
                <button
              ...
              {isDraft && can('stock_opname.delete') && (
                <button
  ...
  const renderEmptyState = useCallback(() => (
    <div className="text-center py-12">
      ...
      <h3 className="mt-2 text-sm font-medium text-gray-900">Tidak ada stock opname</h3>
      <p className="mt-1 text-sm text-gray-500">
        {statusFilter === 'all'
          ? 'Belum ada stock opname yang dibuat. Mulai dengan membuat stock opname baru.'
          : `Tidak ada stock opname dengan status ${statusFilter}.`}
      </p>
      {can('stock_opname.create') && statusFilter === 'all' && (
        <div className="mt-6">
          <button
      ...
      <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div className="md:flex md:items-center md:justify-between mb-6">
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Kelola stock opname dan tracking pencatatan produk
            </p>
          </div>
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-5">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">Total Shortage</dt>
                    <dd className="text-lg font-medium text-gray-900">
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-5">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">Total Surplus</dt>
                    <dd className="text-lg font-medium text-gray-900">
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-5">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">Total Matching</dt>
                    <dd className="text-lg font-medium text-gray-900">
        ...
        {/* Stock Opname Records List */}
        <div className="bg-white shadow sm:rounded-lg p-4 sm:p-6">
          {stockOpnameRecords && stockOpnameRecords.length > 0 ? (
            <div>
              {stockOpnameRecords.map(renderStockOpnameCard)}
          ...
          <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
              ...
              <div className="bg-gray-50 rounded-md p-3 mb-4">
                ...
                <p className="text-sm text-gray-600">
                  <span className="font-medium">Total Items:</span> {deleteConfirm.items_count}
                </p>
                {deleteConfirm.note && (
                  <p className="text-sm text-gray-600">
