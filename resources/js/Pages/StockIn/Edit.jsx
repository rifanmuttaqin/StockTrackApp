...
/**
 * Komponen Edit untuk mengedit Stock Masuk
 *
 * @component
 * @returns {JSX.Element}
 */
const Edit = ({ stockInRecord }) => {
  const { props } = usePage();
  ...
  const { data, setData, put, post, processing, errors, reset } = useForm({
    date: stockInRecord?.date || new Date().toISOString().split('T')[0],
    note: stockInRecord?.note || '',
    items: stockInRecord?.items?.map((item) => ({
  ...
  const handleUpdateDraft = (e) => {
    ...
    if (stockInRecord?.status === 'submit') {
      setMessage({
        type: 'error',
    ...
    put(route('stock-in.update', stockInRecord.id), {
  ...
  /**
   * Handle submit untuk Submit Stock In
  ...
  const handleSubmit = (e) => {
    ...
    post(route('stock-in.submit', stockInRecord.id), {
      onSuccess: () => {
      ...
      <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 pb-20 sm:pb-0">
        ...
        <div className="md:flex md:items-center md:justify-between mb-6">
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-3">
              ...
              <Badge variant={getStatusBadgeVariant(stockInRecord?.status)}>
                {getStatusLabel(stockInRecord?.status)}
              </Badge>
            </div>
            <p className="mt-1 text-sm text-gray-500">
              Edit data stock masuk dengan ID: {stockInRecord?.id}
            </p>
          </div>
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <MobileForm>
            ...
            <MobileFormSection
              ...
              description="Edit tanggal stock masuk"
            ...
            >
              ...
              >
                <CustomDateInput
                  ...
                  error={errors.date}
                  disabled={stockInRecord?.status === 'submit'}
                />
              </MobileFormField>
                ...
                >
                  <textarea
                    ...
                    rows={3}
                    maxLength={500}
                    placeholder="Masukkan catatan (opsional, maksimal 500 karakter)"
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-500">{stockInRecord?.id}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-500">
                        {stockInRecord?.created_at ? formatDate(stockInRecord.created_at) : '-'}
                      </p>
                    </div>
                    <div>
                      <h4 className="text-sm font-medium text-gray-900">Terakhir Diupdate</h4>
                      <p className="mt-1 text-sm text-gray-500">
                        {stockInRecord?.updated_at ? formatDate(stockInRecord.updated_at) : '-'}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <MobileFormSection
              title="Daftar Varian"
              description={`Edit quantity untuk setiap varian (${stockInRecord?.items?.length || 0} varian)`}
              className="p-6 border-b border-gray-200"
            >
              {stockInRecord?.items?.length > 0 ? (
                <div className="space-y-4">
                  {stockInRecord.items.map((item, index) => {
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        <span className="text-sm font-medium text-blue-900">
                          Total Varian: {stockInRecord.items.length}
                        </span>
        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-blue-800">Tips Edit Stock Masuk</h3>
              <div className="mt-2 text-sm text-blue-700">
                <ul className="list-disc pl-5 space-y-1">
                  <li>Quantity tidak boleh negatif</li>
                  <li>Minimal satu varian harus memiliki quantity {'>'} 0</li>
                  <li>Update Draft untuk menyimpan tanpa menambah stock</li>
