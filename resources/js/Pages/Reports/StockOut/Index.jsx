...
/**
 * Stock Out Report Page Component
 * Displays stock out quantities by product variant across dates
 * Desktop-only access
 */
const Index = ({ products, stockOutData, filters, error }) => {
  const { props } = usePage();
  ...
  const allConversions = React.useMemo(() => {
    const map = new Map();
    stockOutData?.products?.forEach(product => {
  ...
  /**
   * Get cell value for stock out quantity
   */
  const getCellValue = (stockOutByDate, date, variant) => {
    const quantity = stockOutByDate?.[date];
        ...
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              Silakan buka aplikasi menggunakan perangkat desktop untuk melihat laporan stock out.
            </p>
          </div>
        </div>
      </AppLayout>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {/* Date Columns */}
                  {stockOutData?.dates && stockOutData.dates.length > 0 ? (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {stockOutData?.products && stockOutData.products.length > 0 ? (
                  stockOutData.products.map((product) => (
                    <React.Fragment key={product.id}>
                      ...
                      {product.variants && product.variants.length > 0 ? (
                        product.variants.map((variant) => (
                          <tr key={variant.id} className="hover:bg-gray-50">
                            ...
                            {stockOutData.dates.map((date) => (
                              ...
                              >
                                {getCellValue(variant.stock_out_by_date, date, variant)}
                              </td>
                            ))}
                            {/* Total (Jumlah) Column */}
                        ...
                        <tr>
                          <td
                            colSpan={stockOutData.dates.length + 3}
                  ...
                  <tr>
                    <td
                      colSpan={(stockOutData?.dates?.length || 0) + 3}
                      className="px-6 py-12 text-center"
                    >
                      <DocumentTextIcon className="mx-auto h-12 w-12 text-gray-400 mb-4" />
                      <h3 className="text-sm font-medium text-gray-900 mb-1">
                        Tidak ada data stock out
                      </h3>
          ...
          {stockOutData?.products && stockOutData.products.length > 0 && (
            <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
              <div className="flex items-center justify-between text-sm text-gray-600">
                <div>
                  <span className="font-medium">Total Produk:</span> {stockOutData.products.length}
                </div>
                <div>
                  <span className="font-medium">Total Variasi:</span>{' '}
                  {stockOutData.products.reduce((sum, product) => sum + (product.variants?.length || 0), 0)}
                </div>
                <div>
                  <span className="font-medium">Rentang Tanggal:</span>{' '}
                  {stockOutData.dates.length > 0 && (
                    <>
                      {formatDate(stockOutData.dates[0])} - {formatDate(stockOutData.dates[stockOutData.dates.length - 1])}
                    </>
                  )}
                </div>
              </div>
