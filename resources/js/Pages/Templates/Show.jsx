...
const Show = ({ template, errors, flash }) => {
  const { props } = usePage();
  ...
  const handleDeleteTemplate = () => {
    if (confirm('Apakah Anda yakin ingin menghapus template ini? Semua varian terkait juga akan dihapus.')) {
      ...
      <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500 flex items-center space-x-2">
                  ...
                  <span>Dibuat pada {formatDate(template?.created_at)}</span>
                </p>
              </div>
            </div>
          </div>
          <div className="mt-4 flex md:mt-0 md:ml-4 space-x-3">
            {!template?.is_active && can('templates.update') && (
              <button
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
          <div className="px-4 py-5 sm:px-6">
            ...
            <p className="mt-1 max-w-2xl text-sm text-gray-500">
              Detail lengkap mengenai template ini
            </p>
          </div>
          <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
            <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <dt className="text-sm font-medium text-gray-500">Nama Template</dt>
                <dd className="mt-1 text-sm text-gray-900 font-semibold">{template?.name}</dd>
              </div>
              <div>
              ...
              <div>
                ...
                <dd className="mt-1">
                  <Badge variant="info">{template?.variants?.length || 0} Varian</Badge>
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Dibuat Pada</dt>
                <dd className="mt-1 text-sm text-gray-900">{formatDate(template?.created_at)}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Terakhir Diupdate</dt>
                <dd className="mt-1 text-sm text-gray-900">{formatDate(template?.updated_at)}</dd>
              </div>
            </dl>
          </div>
        </div>
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
            <div>
              <h3 className="text-lg leading-6 font-medium text-gray-900">
                Daftar Varian dalam Template
              </h3>
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                Semua varian yang terkait dengan template ini
              </p>
            </div>
          </div>
          ...
          {template?.variants && template.variants.length > 0 ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {template.variants.map((variant) => (
                    <tr key={variant.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStockStatusColor(variant.stock_current)}`}>
            ...
            <div className="px-4 py-8 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Template ini belum memiliki varian. Edit template untuk menambahkan varian.
              </p>
              {can('templates.edit') && (
                <Link
                  href={route('templates.edit', template.id)}
              ...
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg font-medium text-gray-900">
                      {template?.variants?.reduce((sum, v) => sum + v.stock_current, 0) || 0} unit
                    </dd>
                  </dl>
                </div>
              </div>
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-blue-700">
                <ul className="list-disc pl-5 space-y-1">
                  <li>Gunakan tombol "Edit Template" untuk mengubah informasi template dan menambah varian baru</li>
                  <li>Gunakan tombol "Set Aktif" untuk mengaktifkan template ini</li>
